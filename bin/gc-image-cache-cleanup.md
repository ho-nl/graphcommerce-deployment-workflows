...
The cache is read from `$GC_DEPLOY_DIR/graphcommerce_main/shared/.next/cache/images`, so only the `_main`
`applicationSuffixId` is supported.
...
- Runs append JSON lines to `$GC_DEPLOY_DIR/logs/image-cache-cleanup.log`: `start` (parameters), `finish`
  (statistics), `skipped` (another run is still busy) or `error`.
